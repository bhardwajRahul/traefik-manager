...
Every removal is listed under **Settings - Backups - Certificates**, and restoring one puts that copy of `acme.json` back and restarts Traefik. The tab only appears once there is a certificate backup to show. Retention follows the same setting as the other backups. When two stores share a file name, their backups carry the store's folder name, so each restores into its own store.
